...
(e.g. [krea2_identity_edit_v1](https://civitai.com/models/2761113)). Port of the
...
- A krea2_edit LoRA at **strength 1.0** (`krea2_identity_edit_v1.safetensors` is in `models/Lora`).

## Usage

1. Enable **Krea2 Identity Edit** (txt2img), drop the source image, write the instruction as the prompt.
2. Pick an **Aspect ratio handling** mode (training pairs are same-size; AR mismatch degrades
   preservation): `match source` resizes the output to the source's AR at your target resolution;
   `crop source to output AR` keeps YOUR width/height and center-crops the reference to fit it.
3. Recommended (from the LoRA author): most edits → Turbo, 8 steps, CFG 1. Removals → Raw, 20 steps,
   CFG 3. Generate at ≤2MP.
...
5. Two-ref LoRAs (experimental upstream): scene image in the first slot, subject in the second.
